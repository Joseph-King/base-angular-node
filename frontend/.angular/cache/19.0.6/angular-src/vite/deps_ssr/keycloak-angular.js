...
var KeycloakService = class _KeycloakService {
  constructor() {
    this._keycloakEvents$ = new import_rxjs.Subject();
  }
  /**
  ...
  addTokenToHeader(headers = new HttpHeaders()) {
    return (0, import_rxjs.from)(this.getToken()).pipe((0, import_operators.map)((token) => token ? headers.set(this._authorizationHeaderName, this._bearerPrefix + token) : headers));
  }
  /**
...
var KeycloakBearerInterceptor = class _KeycloakBearerInterceptor {
  ...
  intercept(req, next) {
    ...
    return (0, import_rxjs.combineLatest)([(0, import_rxjs.from)(this.conditionallyUpdateToken(req)), (0, import_rxjs.of)(this.keycloak.isLoggedIn())]).pipe((0, import_operators.mergeMap)(([_, isLoggedIn]) => isLoggedIn ? this.handleRequestWithTokenHeader(req, next) : next.handle(req)));
  }
  /**
  ...
  handleRequestWithTokenHeader(req, next) {
    return this.keycloak.addTokenToHeader(req.headers).pipe((0, import_operators.mergeMap)((headersWithBearer) => {
  ...
  startMonitoring() {
    ...
    this.ngZone.runOutsideAngular(() => {
      ...
      events.forEach((event) => {
        (0, import_rxjs.fromEvent)(window, event).pipe((0, import_operators.debounceTime)(300), (0, import_operators.takeUntil)(this.destroy$)).subscribe(() => this.updateLastActivity());
      });
    });
  }
  /**
...
var customBearerTokenInterceptor = (req, next) => {
  ...
  return (0, import_rxjs.from)(conditionallyUpdateToken(req, keycloak, matchingCondition)).pipe((0, import_rxjs.mergeMap)(() => keycloak.authenticated ? addAuthorizationHeader(req, next, keycloak, matchingCondition) : next(req)));
};
